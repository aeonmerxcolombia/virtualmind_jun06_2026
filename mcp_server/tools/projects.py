...
def create_project(
    ...
    try:
        ...
        return {"success": True, "project_id": project_id, "name": name, "message": f"Proyecto '{name}' creado correctamente"}
...
def add_participant(project_id: int, user_uid: str, role_id: int = 1) -> Dict[str, Any]:
    """Agrega un participante a un proyecto."""
    db = SessionLocal()
    try:
        existing = db.execute(text("""
            SELECT id FROM proyecto_participantes
            WHERE project_id = :pid AND user_uid = :uid
        """), {"pid": project_id, "uid": user_uid}).fetchone()
        if existing:
            return {"success": True, "message": "El usuario ya es participante de este proyecto"}

        db.execute(text("""
            INSERT INTO proyecto_participantes (project_id, user_uid, role_id)
            VALUES (:pid, :uid, :rid)
        """), {"pid": project_id, "uid": user_uid, "rid": role_id})
        db.commit()
        return {"success": True, "message": "Participante agregado correctamente"}
    except Exception as e:
        db.rollback()
        return {"error": str(e)}
    finally:
...
def get_participants(project_id: int) -> List[Dict[str, Any]]:
    """Obtiene los participantes de un proyecto con datos de usuario."""
    db = SessionLocal()
    try:
        result = db.execute(text("""
            SELECT pp.id, pp.project_id, pp.user_uid, pp.role_id,
                   u.nombre, u.email
            FROM proyecto_participantes pp
            LEFT JOIN usuarios u ON pp.user_uid = u.uid
