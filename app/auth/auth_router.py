...
class FaceLoginRequest(BaseModel):
    email: EmailStr
    embedding: List[float] = Field(..., description="Vector facial capturado en tiempo real")
...
# ==========================================
def compare_face_embeddings(known_embedding: List[float], new_embedding: List[float], threshold: float = 0.55) -> bool:
    ...
    distance = math.sqrt(sum((a - b) ** 2 for a, b in zip(known_embedding, new_embedding)))
...
def get_geolocation(ip: str) -> dict:
    try:
        ...
        resp = httpx.get(f"http://ip-api.com/json/{ip}?fields=status,country,city,lat,lon", timeout=2)
        data = resp.json()
...
def registrar_auditoria(db: Session, user_uid: str, ip: str, ciudad: str = None, pais: str = None, lat: float = None, lon: float = None):
    try:
...
def register_user(request: UserCreate, db: Session = Depends(get_db)):
    ...
    try:
        hv = db.query(HojaVida).filter(HojaVida.email == request.email.strip().lower()).first()
        if hv:
...
def register_face(request: FaceRegisterRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Error al guardar biometría: {str(e)}")

    return {"message": "Vector biométrico registrado exitosamente", "status": "ok"}


@router.post("/face/login")
def login_face(request_data: FaceLoginRequest, req: Request, db: Session = Depends(get_db)):
    ...
    if not user.face_embedding:
        raise HTTPException(status_code=400, detail="No tienes un rostro registrado. Inicia con tu correo y regístralo.")

    is_match = compare_face_embeddings(known_embedding=user.face_embedding, new_embedding=request_data.embedding)
    
    if not is_match:
        raise HTTPException(status_code=401, detail="Rostro no reconocido. Acceso denegado.")
    ...
    audit_id = registrar_auditoria(
        ...
        lat=float(req.headers.get("X-Latitude")) if req.headers.get("X-Latitude") else None,
        lon=float(req.headers.get("X-Longitude")) if req.headers.get("X-Longitude") else None
    )
...
@router.post("/login")
def login_user(request: UserLogin, req: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.email == request.email).first()
    ...
    try:
        query = text("UPDATE usuarios SET two_factor_code = :code, two_factor_expires = :exp WHERE email = :email")
        db.execute(query, {"code": otp_code, "exp": expires, "email": user.email})
        db.commit()
    except Exception as e:
        print(f"Error DB al guardar OTP: {e}")
        raise HTTPException(status_code=500, detail="Error interno preparando autenticación")
...
def verify_2fa_code(request: Request, verify_data: Verify2FARequest, db: Session = Depends(get_db)):
    ...
    if not user_data['two_factor_code']:
        raise HTTPException(status_code=400, detail="No se ha solicitado inicio de sesión o el código ya fue usado")
    ...
    db.execute(text("UPDATE usuarios SET two_factor_code = NULL, two_factor_expires = NULL WHERE email = :email"),
...
@router.post("/activate-client", summary="Activar cuenta de cliente y establecer contraseña")
def activate_client(
    ...
    row = db.execute(
        text("SELECT * FROM password_reset_tokens WHERE token = :token AND code = 'activation' AND used = 0 AND (expires_at IS NULL OR expires_at > NOW())"),
        {"token": request.token}
    ).fetchone()
