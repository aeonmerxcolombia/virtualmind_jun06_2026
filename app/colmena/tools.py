    ...
    def list_tools(self, role: str = None) -> List[Dict[str, Any]]:
        ...
        for t in self.tools.values():
            if role and t.required_roles:
                if role not in t.required_roles and "superadmin" not in t.required_roles:
                    continue
...
def _fmt_fields(params: dict) -> str:
    ...
    for name, p in params.items():
        ...
        parts.append(f"  - {name} ({p['type']}) [{label}]{enum_hint}: {p['description']}")
    return "\n".join(parts)
...
def _build_fields_tools(registry: ToolRegistry):
    ...
    async def list_available_fields(bridge: ApiBridge, entity_type: str):
        ...
        fields_map = {
            "project": {
                "name": _build_param("string", "Nombre del proyecto", True, example="Curso de Python"),
                "client_id": _build_param("string", "ID o email del cliente", True, example="maria@ejemplo.com"),
                "tipo_proyecto": _build_param("string", "Tipo de proyecto", True, enum=["Curso", "Diplomado", "Taller", "Manual", "Multimedia", "Otro"]),
                "description": _build_param("string", "Descripcion del proyecto", False, default=""),
                "idioma": _build_param("string", "Idioma principal", False, default="Espanol"),
                "start_date": _build_param("string", "Fecha de inicio (YYYY-MM-DD)", True),
                "end_date": _build_param("string", "Fecha de fin (YYYY-MM-DD)", False),
                "estado": _build_param("string", "Estado del proyecto", False, default="Planificado", enum=["Planificado", "Activo", "En pausa", "Completado", "Cancelado"]),
            },
            "task": {
                "titulo": _build_param("string", "Titulo de la tarea", True, example="Crear modulo 1"),
                "project_id": _build_param("integer", "ID del proyecto al que pertenece", True),
                "descripcion": _build_param("string", "Descripcion detallada", False, default=""),
                "asignado_a": _build_param("string", "User ID del responsable", False),
                "prioridad": _build_param("string", "Prioridad", False, default="media", enum=["baja", "media", "alta", "critica"]),
                "estado": _build_param("string", "Estado", False, default="pendiente", enum=["pendiente", "en_progreso", "completada", "cancelada"]),
                "fecha_entrega": _build_param("string", "Fecha de entrega (YYYY-MM-DD)", False),
            },
            "client": {
                "email": _build_param("string", "Correo electronico del cliente", True, example="cliente@ejemplo.com"),
                "password": _build_param("string", "Contrasena (se genera automaticamente si no se envia)", False),
                "profile": _build_param("object", "Objeto con datos del perfil: {pnombre, snombre, papellido, sapellido, telefono, direccion, ciudad, pais, tipo_entidad}", True),
            },
            "user": {
                ...
                "rol": _build_param("string", "Rol a asignar", False, default="registrado"),
            },
            "resource": {
                "filename": _build_param("string", "Nombre del archivo", True),
                "file_type": _build_param("string", "Tipo de archivo", True, enum=["pdf", "image", "video", "audio", "model"]),
                "category": _build_param("string", "Categoria", True, example="imagenes-ia"),
            ...
            "module": {
                ...
                "study_plan_id": _build_param("integer", "ID del plan de estudio", True),
                "descripcion": _build_param("string", "Descripcion", False),
            },
        }
        info = fields_map.get(entity_type)
        if not info:
            return {"error": f"Entity type '{entity_type}' no soportado", "available": list(fields_map.keys())}
    ...
    registry.register(ColmenaTool(
        ...
        list_available_fields,
        {"entity_type": _build_param("string", "Tipo de entidad: project, task, client, user, resource, study_plan, module", True, example="project")},
        category="meta",
    ))
...
def _build_query_tools(registry: ToolRegistry):
    ...
    registry.register(ColmenaTool(
        ...
        get_projects,
        {
            "estado": _build_param("string", "Filtrar por estado", False, enum=["Planificado", "Activo", "En pausa", "Completado", "Cancelado"]),
            "limit": _build_param("integer", "Maximo de resultados", False, default=50),
        },
        category="projects",
    ))
    ...
    registry.register(ColmenaTool(
        ...
        get_tasks,
        {
            "project_id": _build_param("integer", "ID del proyecto para filtrar", False),
            "limit": _build_param("integer", "Maximo de resultados", False, default=50),
        },
        category="tasks",
    ))
    ...
    registry.register(ColmenaTool(
        ...
        get_resources,
        {
            "category": _build_param("string", "Categoria (imagenes-ia, videos-ia, audios-ia, documentos, modelos-3d)", False),
            "limit": _build_param("integer", "Maximo de resultados", False, default=50),
        },
        category="resources",
    ))
...
def _build_mutation_tools(registry: ToolRegistry):
    ...
    registry.register(ColmenaTool(
        ...
        create_project,
        {
            "name": _build_param("string", "Nombre del proyecto", True, example="Curso de Python Basico"),
            "client_id": _build_param("string", "ID (UUID) del cliente o su email para buscar", True, example="maria@ejemplo.com"),
            "tipo_proyecto": _build_param("string", "Tipo de proyecto", True, enum=["Curso", "Diplomado", "Taller", "Manual", "Multimedia", "Otro"]),
            "description": _build_param("string", "Descripcion del proyecto", False, default=""),
            "idioma": _build_param("string", "Idioma principal", False, default="Espanol"),
            "start_date": _build_param("string", "Fecha de inicio (YYYY-MM-DD)", False),
            "end_date": _build_param("string", "Fecha de fin (YYYY-MM-DD)", False),
            "estado": _build_param("string", "Estado inicial", False, default="Planificado"),
        },
        category="projects",
    ...
    registry.register(ColmenaTool(
        ...
        update_project,
        {
            "project_id": _build_param("integer", "ID del proyecto a actualizar", True),
            "name": _build_param("string", "Nuevo nombre", False),
            "estado": _build_param("string", "Nuevo estado", False, enum=["Planificado", "Activo", "En pausa", "Completado", "Cancelado"]),
            "description": _build_param("string", "Nueva descripcion", False),
            "start_date": _build_param("string", "Nueva fecha inicio (YYYY-MM-DD)", False),
            "end_date": _build_param("string", "Nueva fecha fin (YYYY-MM-DD)", False),
        },
        category="projects",
        requires_confirmation=True,
    ))
    ...
    registry.register(ColmenaTool(
        ...
        create_task,
        {
            "titulo": _build_param("string", "Titulo de la tarea", True, example="Disenar modulo 1"),
            "project_id": _build_param("integer", "ID del proyecto", True),
    ...
    registry.register(ColmenaTool(
        ...
        update_task,
        {
            ...
            "prioridad": _build_param("string", "Nueva prioridad", False, enum=["baja", "media", "alta", "critica"]),
            "estado": _build_param("string", "Nuevo estado", False, enum=["pendiente", "en_progreso", "completada", "cancelada"]),
            "asignado_a": _build_param("string", "Nuevo responsable (user ID)", False),
            "fecha_entrega": _build_param("string", "Nueva fecha entrega (YYYY-MM-DD)", False),
        },
        category="tasks",
        requires_confirmation=True,
    ))
    ...
    registry.register(ColmenaTool(
        ...
        create_resource,
        {
            "filename": _build_param("string", "Nombre del archivo con extension", True, example="imagen1.png"),
            "file_type": _build_param("string", "Tipo de archivo", True, enum=["pdf", "image", "video", "audio", "model"]),
            "category": _build_param("string", "Categoria", True, example="imagenes-ia"),
            "title": _build_param("string", "Titulo del recurso", True),
    ...
    registry.register(ColmenaTool(
        ...
        send_message,
        {
            "receptor_uid": _build_param("string", "UUID del usuario destinatario", True),
    ...
    registry.register(ColmenaTool(
        ...
        close_cronograma,
        {"cronograma_id": _build_param("integer", "ID del cronograma a cerrar", True)},
        category="cronogramas",
        requires_confirmation=True,
    ))
...
def _build_system_tools(registry: ToolRegistry):
    ...
    registry.register(ColmenaTool(
        ...
        heal_service,
        {"service_name": _build_param("string", "Nombre del servicio: virtualmind, mysql, apache2, mcp-gemini, mcp-ollama, ollama", True)},
        category="system",
