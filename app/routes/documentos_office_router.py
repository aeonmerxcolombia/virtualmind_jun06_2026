...
from app.models.documento_office import DocumentoOficina, DocumentoOficinaCompartido, DocumentoVersion, DocumentoRevision, EstadoVersion
...
router = APIRouter(
    prefix="/documentos-office",
    tags=["Documentos Office"]
)

DOCUMENTOS_DIR = "/home/ubuntu/backend/documentos/archivos"
os.makedirs(DOCUMENTOS_DIR, exist_ok=True)
ONLYOFFICE_SECRET = "IaOr1Ch7TeVyCTYuuA70sF8nYoKX26jl"
...
@router.post("/", response_model=DocumentoOfficeOut, status_code=status.HTTP_201_CREATED)
def crear_documento(
    nombre: str = Form(...),
    tipo: str = Form(...),
    ...
    if tipo not in ['docx', 'xlsx', 'pptx']:
        raise HTTPException(status_code=400, detail="Tipo no soportado. Usa: docx, xlsx, pptx")
    ...
    if tipo == 'docx': content = _generate_docx_template(nombre)
    elif tipo == 'xlsx': content = _generate_xlsx_template(nombre)
    elif tipo == 'pptx': content = _generate_pptx_template(nombre)
...
async def get_documento(doc_id: int, db: Session = Depends(get_db), token_data: dict = Depends(verify_token)):
    ...
    if doc.usuario_id != user_uid:
        ...
        compartido = db.query(DocumentoOficinaCompartido).filter(
        ...
        if compartido:
            ...
            try:
                ...
                if owner:
                    ...
                    try:
                        asunto = f"{user_nombre} abrió tu documento compartido: {doc.nombre}"
                        cuerpo = f"""Hola,
...
VirtualMind"""
            ...
            try:
                ...
                revision = db.query(DocumentoRevision).filter(
                    ...
                    DocumentoRevision.estado.in_(["pendiente", "aprobado", "enviado_cliente"])
                ).order_by(DocumentoRevision.id.desc()).first()
...
async def crear_nueva_version(
    ...
    if doc.estado_version == "cerrada":
        raise HTTPException(status_code=400, detail="No se puede crear una nueva versión porque la versión actual está cerrada. La versión cerrada es definitiva.")
...
async def cerrar_version(
    ...
    if not roles_permitidos.intersection(set(roles)) and "superadmin" not in roles:
        raise HTTPException(status_code=403, detail="Solo Autor, Cliente o Superadmin pueden cerrar versiones")
        ...
        if roles_seleccionados or usuarios_seleccionados:
            ...
            for uid in usuarios_seleccionados:
                if uid and uid != user_uid:
                    existe = db.query(AccesoBiblioteca).filter(
            ...
            participantes = db.query(ProyectoParticipante).filter(
                ProyectoParticipante.project_id == doc.project_id
            ).all()
...
async def listar_versiones(
    ...
    versiones = db.query(DocumentoVersion).filter(
        DocumentoVersion.documento_id == doc_id
    ).order_by(DocumentoVersion.id.desc()).all()
...
async def descargar_version(
    ...
    if not os.path.exists(filepath):
        raise HTTPException(status_code=404, detail="Archivo de versión no encontrado en disco")
...
def eliminar_documento(doc_id: int, db: Session = Depends(get_db), token_data: dict = Depends(verify_token)):
    doc = db.query(DocumentoOficina).filter(DocumentoOficina.id == doc_id).first()
    if not doc: raise HTTPException(status_code=404, detail="No encontrado")
...
async def get_archivo(filename: str):
    ...
    response = FileResponse(filepath, media_type=media_types.get(ext, 'application/octet-stream'), filename=filename)
    response.headers['Access-Control-Allow-Origin'] = '*'
    response.headers['X-Frame-Options'] = 'ALLOWALL'
    return response

@router.post("/callback/{filename}")
async def onlyoffice_callback(filename: str, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        if status == 2 or status == 6:
            ...
            if download_url:
                ...
                if response.status_code == 200:
                    ...
                    # Update MySQL
                    doc = db.query(DocumentoOficina).filter(DocumentoOficina.filename == filename).first()
                    if doc:
...
async def config_archivo_from_drive(req: ConfigArchivoRequest, request: Request, db: Session = Depends(get_db), token_data: dict = Depends(verify_token)):
    ...
    if ext not in ('docx', 'xlsx', 'pptx'):
        raise HTTPException(status_code=400, detail="Tipo de archivo no soportado para OnlyOffice")
    ...
    config = {
        ...
        "editorConfig": {
            ...
            "customization": {"forcesave": True, "autosave": True, "about": False, "feedback": False}
        }
    }
...
async def get_onlyoffice_config(doc_id: int, request: Request, db: Session = Depends(get_db), token_data: dict = Depends(verify_token)):
    doc = db.query(DocumentoOficina).filter(DocumentoOficina.id == doc_id).first()
    if not doc: raise HTTPException(status_code=404, detail="No encontrado")
...
async def upload_documento_v2(
    ...
    return {"message": "Archivo subido exitosamente", "id": doc.id, "nombre": nombre, "tipo": ext}
...
def mis_documentos_compartidos(
    ...
    compartidos = db.query(DocumentoOficina).join(
        DocumentoOficinaCompartido,
        DocumentoOficina.id == DocumentoOficinaCompartido.documento_id
    ).filter(
        DocumentoOficinaCompartido.email_destino == user_email
    ).all()
...
async def compartir_documento(
    ...
    existente = db.query(DocumentoOficinaCompartido).filter(
        DocumentoOficinaCompartido.documento_id == doc_id,
        DocumentoOficinaCompartido.email_destino == email_destino
    ).first()
    
    if existente:
        raise HTTPException(status_code=400, detail="Ya está compartido con este usuario")
...
async def enviar_a_revision(
    ...
    if not coordinador_email:
        raise HTTPException(status_code=422, detail="email del coordinador es requerido")
    ...
    if doc.estado_version == "cerrada":
        raise HTTPException(status_code=400, detail="No se puede enviar una versión cerrada a revisión")
    ...
    if not es_coordinador and not es_superadmin:
        raise HTTPException(status_code=403, detail="El usuario seleccionado no tiene rol de Coordinador")
...
async def aprobar_revision(
    ...
    if "coordinador" not in roles and "superadmin" not in roles:
        raise HTTPException(status_code=403, detail="Solo el Coordinador puede aprobar revisiones")
    ...
    if revision:
        ...
        revision.comentarios = (revision.comentarios or "") + f"\n[Aprobado por coordinador] {body.get('comentarios', '')}"
...
async def devolver_revision(
    ...
    if "coordinador" not in roles and "superadmin" not in roles:
        raise HTTPException(status_code=403, detail="Solo el Coordinador puede devolver documentos")
    ...
    if "coordinador" not in roles and "superadmin" not in roles:
        ...
        if user_obj:
            ...
            if "coordinador" in db_roles or "superadmin" in db_roles:
                roles = db_roles
        if "coordinador" not in roles and "superadmin" not in roles:
            raise HTTPException(status_code=403, detail="Solo el Coordinador puede ver revisiones pendientes")

    revisiones = db.query(DocumentoRevision).filter(
        DocumentoRevision.estado == "pendiente",
        DocumentoRevision.coordinador_id == user_uid
    ).order_by(DocumentoRevision.fecha_envio.desc()).all()

    result = []
    for rev in revisiones:
        doc = db.query(DocumentoOficina).filter(DocumentoOficina.id == rev.documento_id).first()
        if not doc:
            continue
...
async def trazabilidad_documento(
    ...
    for rev in doc.revisiones:
        ...
        if rev.coordinador_visto:
            eventos.append({
                ...
                "fecha": rev.coordinador_visto.isoformat() if rev.coordinador_visto else None,
        ...
        if rev.estado == "aprobado" or rev.estado == "enviado_cliente":
            eventos.append({
                "tipo": "aprobado",
                "fecha": rev.fecha_respuesta.isoformat() if rev.fecha_respuesta else None,
                "descripcion": f"Aprobado por coordinador y enviado al cliente" + (f" (ID: {rev.cliente_id})" if rev.cliente_id else ""),
                "version": rev.version
            })
            if rev.cliente_visto:
                eventos.append({
                    "tipo": "visto_cliente",
                    "fecha": rev.cliente_visto.isoformat() if rev.cliente_visto else None,
    ...
    for comp in doc.compartidos:
        eventos.append({
            "tipo": "compartido",
            "fecha": comp.fecha_compartido.isoformat() if comp.fecha_compartido else None,
...
async def auditar_documento_ia(
    ...
    revisiones = db.query(DocumentoRevision).filter(
        DocumentoRevision.documento_id == doc_id
    ).order_by(DocumentoRevision.id.desc()).all()
    ...
    contexto_cambios = "\n".join(cambios_solicitados) if cambios_solicitados else "No hay cambios solicitados registrados."
...
{contexto_cambios[:5000]}"""
        ...
        try:
            ...
            resultado = {"error": "La IA no devolvió JSON válido", "raw": resultado_texto}
