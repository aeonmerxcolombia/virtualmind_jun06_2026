...
async def obtener_laboratorio(
    ...
    return {
        ...
        "estado": lab.estado,
        "fecha_creacion": lab.fecha_creacion.isoformat() if lab.fecha_creacion else None
    }
...
async def crear_experimento(
    ...
    return {
        "id": db_exp.id,
        "nombre": db_exp.nombre,
        "mensaje": "Experimento creado"
    }
...
async def obtener_guia_vr(
    ...
    crear_keywords = ['créalo', 'crear laboratorio', 'nuevo laboratorio', 'créate', 'genéralo', 'crea un']
...
Responde SOLO con el JSON, sin texto adicional."""
        ...
        try:
            ...
            data = response.json()
            texto_respuesta = data.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", "{}")
...
Responde de manera clara, concisa y educativa. Si el usuario muestra interés en un tema en particular, ofrécete a crear un laboratorio de aprendizaje virtual sobre ese tema."""
    ...
    try:
        response = requests.post(url, headers={"Content-Type": "application/json"}, json=body, timeout=60)
        data = response.json()
        respuesta = data.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", "Sin respuesta")
        return {
